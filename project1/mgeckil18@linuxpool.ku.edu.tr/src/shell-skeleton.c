...
int process_command(struct command_t *command) {
	...
	if (strcmp(command->name, "wiseman") == 0) {
        return handle_wiseman_command(command);
    }
...
int handle_wiseman_command(struct command_t *command) {
...
//     const char *commands[] = {"cd", "exit", "uniq", "wiseman", "psvis", "hdiff", NULL};  // Add all your commands here
//     int partial = 0;
